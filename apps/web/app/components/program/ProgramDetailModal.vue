...
const daysUntil = computed(() => {
  ...
  const due = new Date(props.program.applicationDeadline)
...
const groupedRequirements = computed(() => {
  ...
  const map: Record<string, typeof props.program.requirements> = {}
  props.program.requirements.forEach((r) => {
...
<template>
  <UiModal :open="open" :width="780" :label="program?.name ?? 'Program details'" @close="$emit('close')">
    <div v-if="pending" class="flex flex-col items-center justify-center gap-3 px-7 py-24">
    ...
    <template v-else-if="program">
      <!-- Navy header -->
      <div class="relative px-7 pb-[18px] pt-[22px]" style="background: #0B1F44; color: #fff">
        <button
          type="button"
          aria-label="Close"
          class="absolute right-3.5 top-3.5 flex h-8 w-8 items-center justify-center rounded-lg text-white"
        ...
        <h2 class="m-0 text-[22px] font-semibold leading-snug tracking-[-0.014em]">{{ program.name }}</h2>
